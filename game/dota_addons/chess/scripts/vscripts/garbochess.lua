...
g_yieldfinCnt = 10;	-- number of moves to analyze before yielding to check timeout
...
function AllCutNode(ply, depth, beta, allowNull)
  ...
  g_moveTime = Time() - g_startTime
  if (g_moveTime > g_timeout) then
    -- Time cutoff
    g_searchValid = false;
    return beta - 1;
  end
  ...
  if (g_finCnt % g_yieldfinCnt == 0) then
    coroutine.yield()
  end
...
function SetMaxMoveAnalysis(value)
  g_maxfinCnt = value
end

function SetYieldCount(value)
  g_yieldfinCnt = value
end
--[[
